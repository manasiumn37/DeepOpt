%This function implements the processing when only x and y effective is
%candidate for the last subvolume along the F-direction of XYFZ branch
function [cycle_count_sub, SRAM_Access_sub, DRAM_Access_sub] = Asym_XYeffect_XFYZ (Layer_param, Hardware_param, Tech_param)
...
Nos_of_filter = Layer_param(8); % This is the new_Nos_of_filter with which this function is called
stride = Layer_param(9);

%Calling the X->F->Y->Z branch to perform computation with new_Nos_of_filter
...
%keeping the subscript xyzF since this is the same code as the asymmetric top file of the xyzF branch where only candidate for effective is x & y
...
if (done_flag == 0) && (ofmap_height_effective < ofmap_height)
    ...
    [XY_effective, cycle_count, SRAM_Access, DRAM_Access] = XtoFtoYtoZ(Layer_param, Hardware_param, Tech_param);
...
%Variables to return
cycle_count_sub = [cycle_count_ideal_xyzF DRAM_stall_xyzF cycle_to_write_ofmap];
SRAM_Access_sub = [Access_SRAM_filter_xyzF Access_SRAM_ifmap_xyzF Access_SRAM_psum_xyzF];  % in Nos of element, not bit
DRAM_Access_sub = [Access_DRAM_filter_xyzF Access_DRAM_ifmap_xyzF Access_DRAM_ofmap_xyzF]; % in Nos of element, not bit
