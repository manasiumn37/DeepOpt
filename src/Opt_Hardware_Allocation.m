function Pmat_L2Lbest = Opt_Hardware_Allocation (Hardware_param, Tech_param, Pmat_flag, Network_flag)

%This file generates the Performance Metric (Pmat) of a network while using layer-specific optimal branches (LOS) to compute the full network.
...
Energy_Add32 = Tech_param(1);      % for 32 bit
Energy_Mul8 = Tech_param(2);       % for 8 bit
...
%% Performing the computation with layer to layer best branch
for i = 1:1:length(Layer)
    ...
    if (fc_flag(i) == 0)  % Conv Layers
        ...
        MAC_Energy_layer = Nos_of_MAC_layer * (Energy_Add32 + Energy_Mul8); % in Joule, change this if bit width changes
...
%             end
        ...
        elseif (batch_size > 1) % with batching
            ...
            Nos_of_MAC_layer = filter_height * filter_width * Nos_of_channel * ofmap_height * ofmap_width * Nos_of_filter;
            MAC_Energy_layer = Nos_of_MAC_layer * (Energy_Add32 + Energy_Mul8); % in Joule, change this if bit width changes
            % for each MAC, one filter element read, one ifmap element read, one psum element read, and one psum element write occur
            RF_Energy_layer = (E_RF_to_ALU * bw_filter + E_RF_to_ALU * bw_ifmap + 2 * E_RF_to_ALU * bw_psum) * Nos_of_MAC_layer ; % in Joule, 
...
%             % (for the zxyF branch ideally it should be zNF, bit since did not implemented that as not candidate, using NzF)
...
% Result using layer to layer best branching
Best_Pmat_all_layer = Sorted_Pmat_all_branch(:,1);
Pmat_L2Lbest = sum(Best_Pmat_all_layer);             % performance metric based on layer-specific optimal branching
