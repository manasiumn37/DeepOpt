function [cycle_count_layer, SRAM_Access_layer, DRAM_Access_layer] = Asymmetric_XFYZ(Layer_param, Hardware_param, Tech_param)
...
% For XFYZ branch: this function is the top level wrapper to cover all the iterations to compute the full ofmap volume of a layer
...
%% Calling the X->F->Y->Z branch to perform computation
[XFY_effective, cycle_count, SRAM_Access, DRAM_Access] = XtoFtoYtoZ(Layer_param, Hardware_param, Tech_param);

% Getting the value of the x,y,F effective parameters
ofmap_width_effective = XFY_effective(1);
ofmap_height_effective = XFY_effective(2);
ifmap_width_effective = XFY_effective(3);
ifmap_height_effective = XFY_effective(4);
F_effective = XFY_effective(5);
...
if (ofmap_width_effective == ofmap_width) && (ofmap_height_effective == ofmap_height) && (F_effective == Nos_of_filter)
    ...
    cycle_count_ideal_xFyz = cycle_count(1);
    DRAM_stall_xFyz = cycle_count(2);
    cycle_to_write_ofmap = cycle_count(3);

    Access_SRAM_filter_xFyz = SRAM_Access(1); % in Nos of element, not bit
    Access_SRAM_ifmap_xFyz = SRAM_Access(2);
    Access_SRAM_psum_xFyz = SRAM_Access(3);

    Access_DRAM_filter_xFyz = DRAM_Access(1);
    Access_DRAM_ifmap_xFyz = DRAM_Access(2);
    Access_DRAM_ofmap_xFyz = DRAM_Access(3);
else
    % One or more effective had triggered. Lets start with assigining zero to all the computation variables
    done_flag = 0;
    
    cycle_count_ideal_xFyz = 0;
    DRAM_stall_xFyz = 0;
    cycle_to_write_ofmap = 0;

    Access_SRAM_filter_xFyz = 0; 
    Access_SRAM_ifmap_xFyz = 0;
    Access_SRAM_psum_xFyz = 0;

    Access_DRAM_filter_xFyz = 0;
    Access_DRAM_ifmap_xFyz = 0;
    Access_DRAM_ofmap_xFyz = 0;
end
...
if (ofmap_width_effective < ofmap_width) % X_effective triggers, so Y_effective = 1 and F_effective = unitvol_nosof_3D_filter
    ...
    cycle_count_ideal_xFyz = cycle_count_ideal_xFyz * ofmap_height * pass_x;
    DRAM_stall_xFyz = DRAM_stall_xFyz * ofmap_height * pass_x;
    cycle_to_write_ofmap = cycle_to_write_ofmap * ofmap_height * pass_x;
        
    Access_SRAM_filter_xFyz = Access_SRAM_filter_xFyz * ofmap_height * pass_x; % in Nos of element, not bit
    Access_SRAM_ifmap_xFyz = Access_SRAM_ifmap_xFyz * ofmap_height * pass_x;
    Access_SRAM_psum_xFyz = Access_SRAM_psum_xFyz * ofmap_height * pass_x;

    Access_DRAM_filter_xFyz = Access_DRAM_filter_xFyz * ofmap_height * pass_x;
    Access_DRAM_ifmap_xFyz = Access_DRAM_ifmap_xFyz * ofmap_height * pass_x;
    Access_DRAM_ofmap_xFyz = Access_DRAM_ofmap_xFyz * ofmap_height * pass_x;
    ...
    new_ifmap_width = (new_ofmap_width - 1) * stride + filter_width;
    new_Nos_of_filter = F_effective; %fixing the 3rd dimension of ofmap to F-effective
    ...
    elseif (new_ofmap_width > 0) && (ofmap_height_effective == ofmap_height)
        ...
        cycle_count_ideal_xFyz = cycle_count_ideal_xFyz + cycle_count(1);
        DRAM_stall_xFyz = DRAM_stall_xFyz + cycle_count(2);
        cycle_to_write_ofmap = cycle_to_write_ofmap + cycle_count(3);

        Access_SRAM_filter_xFyz = Access_SRAM_filter_xFyz + SRAM_Access(1); % in Nos of element, not bit
        Access_SRAM_ifmap_xFyz = Access_SRAM_ifmap_xFyz + SRAM_Access(2);
        Access_SRAM_psum_xFyz = Access_SRAM_psum_xFyz + SRAM_Access(3);

        Access_DRAM_filter_xFyz = Access_DRAM_filter_xFyz + DRAM_Access(1);
        Access_DRAM_ifmap_xFyz = Access_DRAM_ifmap_xFyz + DRAM_Access(2);
        Access_DRAM_ofmap_xFyz = Access_DRAM_ofmap_xFyz + DRAM_Access(3);
...
if (new_ofmap_width > 0) && (ofmap_height_effective < ofmap_height)
    ...
    % Here, since this is x->F->y->z branch, if F_effective is already triggered then Y_effective = 1. If F_effective did not trigger then Y_effective >= 1.
    % For simplicity and commonality, going towards y-direction first in the asymmetric box although F-direction comes before y-direction in this branch.
    ...
    cycle_count_ideal_xFyz_sub = cycle_count(1);
    DRAM_stall_xFyz_sub = cycle_count(2);
    cycle_to_write_ofmap_sub = cycle_count(3);
    
    Access_SRAM_filter_xFyz_sub = SRAM_Access(1); % in Nos of element, not bit
    Access_SRAM_ifmap_xFyz_sub = SRAM_Access(2);
    Access_SRAM_psum_xFyz_sub = SRAM_Access(3);

    Access_DRAM_filter_xFyz_sub = DRAM_Access(1);
    Access_DRAM_ifmap_xFyz_sub = DRAM_Access(2);
    Access_DRAM_ofmap_xFyz_sub = DRAM_Access(3);
    ...
    cycle_count_ideal_xFyz = cycle_count_ideal_xFyz + cycle_count_ideal_xFyz_sub * pass_y;
    DRAM_stall_xFyz = DRAM_stall_xFyz + DRAM_stall_xFyz_sub * pass_y;
    cycle_to_write_ofmap = cycle_to_write_ofmap + cycle_to_write_ofmap_sub * pass_y;

    Access_SRAM_filter_xFyz = Access_SRAM_filter_xFyz + Access_SRAM_filter_xFyz_sub * pass_y; % in Nos of element, not bit
    Access_SRAM_ifmap_xFyz = Access_SRAM_ifmap_xFyz + Access_SRAM_ifmap_xFyz_sub * pass_y;
    Access_SRAM_psum_xFyz = Access_SRAM_psum_xFyz + Access_SRAM_psum_xFyz_sub * pass_y;

    Access_DRAM_filter_xFyz = Access_DRAM_filter_xFyz + Access_DRAM_filter_xFyz_sub * pass_y;
    Access_DRAM_ifmap_xFyz = Access_DRAM_ifmap_xFyz + Access_DRAM_ifmap_xFyz_sub * pass_y;
    Access_DRAM_ofmap_xFyz = Access_DRAM_ofmap_xFyz + Access_DRAM_ofmap_xFyz_sub * pass_y;
...
if (F_effective < Nos_of_filter)
    ...
    if (cycle_count_ideal_xFyz == 0) % This means neither x or y effective triggered, only F-effective triggered, Hence the variable values are zero
    ...
    cycle_count_ideal_xFyz = cycle_count_ideal_xFyz * pass_F;
    DRAM_stall_xFyz = DRAM_stall_xFyz * pass_F;
    cycle_to_write_ofmap = cycle_to_write_ofmap * pass_F;
        
    Access_SRAM_filter_xFyz = Access_SRAM_filter_xFyz * pass_F; % in Nos of element, not bit
    Access_SRAM_ifmap_xFyz = Access_SRAM_ifmap_xFyz * pass_F;
    Access_SRAM_psum_xFyz = Access_SRAM_psum_xFyz * pass_F;

    Access_DRAM_filter_xFyz = Access_DRAM_filter_xFyz * pass_F;
    Access_DRAM_ifmap_xFyz = Access_DRAM_ifmap_xFyz * pass_F;
    Access_DRAM_ofmap_xFyz = Access_DRAM_ofmap_xFyz * pass_F;
    
    % The remaining part along the F-direction which is not processed yet
    new_Nos_of_filter = Nos_of_filter - F_effective * pass_F;
    
    if (new_Nos_of_filter == 0) % computation is done for the full xFy direction if there is no remaining part along the F-direction
        disp('computation calculation done for the full xFy direction')
        done_flag = 1;
    else
        % Computation for the remaining part in the F-direction
        % This situation of last subvolume of F comes only when F-effective triggers. Hence can write inside the F-effective condition
        ...
        [cycle_count_sub, SRAM_Access_sub, DRAM_Access_sub] = Asym_XYeffect_XFYZ(Layer_param, Hardware_param, Tech_param);   
    
        cycle_count_ideal_xFyz = cycle_count_ideal_xFyz + cycle_count_sub(1);
        DRAM_stall_xFyz = DRAM_stall_xFyz + cycle_count_sub(2);
        cycle_to_write_ofmap = cycle_to_write_ofmap + cycle_count_sub(3);

        Access_SRAM_filter_xFyz = Access_SRAM_filter_xFyz + SRAM_Access_sub(1); % in Nos of element, not bit
        Access_SRAM_ifmap_xFyz = Access_SRAM_ifmap_xFyz + SRAM_Access_sub(2);
        Access_SRAM_psum_xFyz = Access_SRAM_psum_xFyz + SRAM_Access_sub(3);

        Access_DRAM_filter_xFyz = Access_DRAM_filter_xFyz + DRAM_Access_sub(1);
        Access_DRAM_ifmap_xFyz = Access_DRAM_ifmap_xFyz + DRAM_Access_sub(2);
        Access_DRAM_ofmap_xFyz = Access_DRAM_ofmap_xFyz + DRAM_Access_sub(3);
    end
end
...
% 
% Access_SRAM_filter_xFyz
% Access_SRAM_ifmap_xFyz
% Access_SRAM_psum_xFyz
% 
% Access_DRAM_filter_xFyz
% Access_DRAM_ifmap_xFyz
% Access_DRAM_ofmap_xFyz

cycle_count_layer = [cycle_count_ideal_xFyz DRAM_stall_xFyz cycle_to_write_ofmap];
SRAM_Access_layer = [Access_SRAM_filter_xFyz Access_SRAM_ifmap_xFyz Access_SRAM_psum_xFyz];  % in Nos of element, not bit
DRAM_Access_layer = [Access_DRAM_filter_xFyz Access_DRAM_ifmap_xFyz Access_DRAM_ofmap_xFyz]; % in Nos of element, not bit
